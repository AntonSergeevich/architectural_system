...
#
# Если лендинг «Твёрдого знака» ещё не выкачен целиком — уберите третью
# строку. Ссылка на недоделанную страницу стоит дороже, чем её отсутствие.
SYSTEMS = [
    (DARYA, "Архитектор-дизайнер", "конструктор проекта прямо на главной"),
    (LINGUICH, "Языковая школа", "тест уровня, кабинеты, расписание"),
    (TVERDYY, "Центр семейного обучения", "кейс — третий в очереди"),
]
...
def story_09_cta():
    ...
    rule(d, M, y, 96, BRAND, h=5)
    y += 40
    block(d, "Архитектор. Языковая школа. Центр\nсемейного обучения. Один принцип:\nсистема работает вместо вас.",
