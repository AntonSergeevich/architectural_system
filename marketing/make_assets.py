...
def card_reglament():
    ...
    rule(d, CM, y, 96, BRAND, h=5)
    y += 42
    block(d, "«Отвечу примерно через час», — сказала Дарья.\nРовно через час пришло: «вы мне так\nи не ответили». Слово «примерно» не расслышали.",
    ...
    block(d, "Срок называет не человек\nв моменте, а правило.",
...
def story_reglament():
    ...
    y = 580
    y = block(d, "«Отвечу примерно через час»,\nсказала Дарья. Ровно через час пришло:\n«вы мне так и не ответили».",
    ...
    y = 1200
    y = block(d, "Теперь срок называет не Дарья в моменте,\nа регламент — один и тот же для всех.",
              font(T_SEMI, 38), (M, y, 1080 - 2 * M), BRAND_600, leading=1.42, tag="rsty-a")

    d.rectangle([M, 1400, 1080 - M, 1660], fill=MARKER)
    block(d, "«Примерно через час» — это\nне оценка. Это обещание,\nи его засекут.",
