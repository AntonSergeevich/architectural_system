...
class User(AbstractUser):
    ...
    # Согласие на обработку персональных данных. На сайте его даёт тот,
    # кто заполняет форму, — но заказчик, которому доступ выдала Дарья,
    # форму не заполнял: он получил логин в мессенджере. В кабинете при
    # этом лежат его телефон, адрес объекта и переписка, то есть ровно
    # персональные данные. Поэтому согласие спрашивается при первом входе,
    # и до него кабинет не открывается.
    data_consent_at = models.DateTimeField(
        "Согласие на обработку данных", null=True, blank=True
    )
